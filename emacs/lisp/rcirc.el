...
If ARG is non-nil, instead prompt for connection parameters."
  ...
  (if arg
	...
	(let* ((server (car c))
        ...
        (processname (format "%s|%s" server user-name))
        contact)
	  (when server
	    (let (connected)
	      (dolist (p (rcirc-process-list))
		(when (string= processname (process-name p))
		  (setq connected p)))
	      (if (not connected)
		  (condition-case e
		      (rcirc-connect server port nick user-name
...
(defun rcirc-connect (server &optional port nick user-name
  ...
  (save-excursion
     ...
     (processname (format "%s|%s" server user-name))
	   (startup-channels startup-channels)
           (process (open-network-stream
                     processname nil server port-number
	...
	(setq rcirc-keepalive-timer
      ...
      (message "Connecting to %s as %s...done" server user-name)
